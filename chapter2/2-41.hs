...
remove_all x s = filter (\y -> not (y `elem` x)) s
...
unique_triplets :: Int -> [[Int]]
unique_triplets n = flat_map (\i -> flat_map (\j -> map (\k -> [i, j, k]) $ remove_all [i,j] [1..n] ) $ remove i [1..n]) [1..n] 

sum_triplets n s = filter (\p -> (p!!0 + p!!1 + p!!2 == s)) $ unique_triplets n

main = do
    print $ unique_pairs 10
    print $ prime_sum_pairs 10
    print $ permutations [1..3]
    print $ unique_triplets 4
    print $ sum_triplets 4 7
